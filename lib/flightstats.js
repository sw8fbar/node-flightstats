...
FlightStats.prototype = {
  ...
  getAirports: function( options, callback ) {
    ...
    if( isLocation ) {
      baseUrl += '/withinRadius/' + options.longitude + '/' + options.latitude + '/' + options.radius
    }
  ...
  schedule: function( options, callback ) {
    ...
    var protocol = options.protocol || 'rest'
    var format = options.format || 'json'
    var baseUrl = 'schedules/' + protocol + '/v1/' + format
    ...
    var day = options.date.getDate()
    var hourOfDay = options.hourOfDay
    ...
    if(hourOfDay && airport) {
      extensions = ['includeDirects'];
      if(direction == 'departing') {
        baseUrl = baseUrl + '/from'+ '/' + airport + '/' + direction + '/' + year + '/' + month + '/' + day + '/' + hourOfDay
      } else {
        baseUrl = baseUrl + '/to'+ '/' + airport + '/' + direction + '/' + year + '/' + month + '/' + day + '/' + hourOfDay
      }

    } else {
      baseUrl = baseUrl + '/flight'+ '/' + carrier + '/' + flightNumber + '/' + direction + '/' + year + '/' + month + '/' + day
    }
    ...
    return this._clientRequest({
      ...
      if( data != null ) {
        ...
        if( options.airport && data.flights && !options.hourOfDay) {
          data.flights = FlightStats.filterByAirport( data.flights, options.airport, options.direction )
        }
      }
  ...
  fids: function( options, callback ) {
    debug( 'fids', options )
    var status = require(flight-status);

    var self = this

    var airport = options.airport;
    var timeWindowBegin = options.timeWindowBegin;
    var timeWindowEnd = options.timeWindowEnd;
    var excludeCargoOnlyFlights = options.excludeCargoOnlyFlights;

    var requestedFields = ''
    if(options.requestedFields && options.requestedFields.length) {
      requestedFields = options.requestedFields[0];
      for (var i=1; i < options.requestedFields.length; i++){
        requestedFields = requestedFields + ','+ options.requestedFields[i]
      }
    } else {
      requestedFields="airlineLogoUrlSvg,airlineCode,flightNumber,city,currentTime,gate,remarks"
    }
    var direction = /^dep/i.test( options.direction ) ?
      'departures' : 'arrivals'
    ...
    var url = 'fids/rest/v1/json' + '/' + options.airport + '/' + direction
    ...
    return this._clientRequest({
      ...
      qs: {
        ...
        excludeCargoOnlyFlights: excludeCargoOnlyFlights,
        timeFormat: 24

      }
    }, function( error, data ) {
      if(data.fidsData){
        data = data.fidsData.map(function(flight){
          flight.status = status[flight.statusCode];
          return flight;
        })
      } else {
        data = [];
